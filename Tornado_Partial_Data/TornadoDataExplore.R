...
ts.plot(ts(apply(intmat,1,mean),start=c(1950)),ylab="Intensity")
...
##############################################################
##############Median Maps for sum(STP) and sumax(STP)#########
par(mfrow=c(2,2),mar=c(2,2,2,1))
for(j in 1:4){
plot(crop(qtsumstpM[[j]],XUS),col=viridis(10),breaks=c(1,5,10,15,20,30,40,50,60,70,80,90))
data(wrld_simpl)
#plot(wrld_simpl,add=TRUE)
map("state",add=TRUE)
...
par(mfrow=c(2,2),mar=c(2,2,2,1))
for(j in 1:4){
  hist(crop(qtsumaxstpM[[j]],XUS),main="")
  title(paste("Median sumax(STP) ,",quarter.name[j],sep=""))
}
#par(mfrow=c(2,2),mar=c(2,2,2,1))
par(mfrow=c(2,2))
...
#
###########Saving Files for Blackwell  Scholars###########
STPSUMAX_1987_Q1=VNc1987[[1]]
STPSUMAX_1987_Q2=VNc1987[[2]]
STPSUMAX_1987_Q3=VNc1987[[3]]
STPSUMAX_1987_Q4=VNc1987[[4]]
save(STPSUMAX_1987_Q1,file="VNc1987Q1.RData")
save(STPSUMAX_1987_Q2,file="VNc1987Q2.RData")
save(STPSUMAX_1987_Q3,file="VNc1987Q3.RData")
save(STPSUMAX_1987_Q4,file="VNc1987Q4.RData")
#
###################################################################
...
save(list2011,file="reports-2011.RData")
###################################################################
###########Saving Files for Blackwell  Scholars###########
STPSUMAX_2011_Q1=VNc2011[[1]]
STPSUMAX_2011_Q2=VNc2011[[2]]
STPSUMAX_2011_Q3=VNc2011[[3]]
STPSUMAX_2011_Q4=VNc2011[[4]]
save(STPSUMAX_2011_Q1,file="VNc2011Q1.RData")
save(STPSUMAX_2011_Q2,file="VNc2011Q2.RData")
save(STPSUMAX_2011_Q3,file="VNc2011Q3.RData")
save(STPSUMAX_2011_Q4,file="VNc2011Q4.RData")
########################################################
#Plot Intensity vs quantiles
dev.new()
par(mfrow=c(2,2),mar=c(2,2,2,1))
